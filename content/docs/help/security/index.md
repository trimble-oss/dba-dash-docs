...
If you want to use [WMI](/docs/help/wmi) collections (optional), the service account will need to be a local admin on the monitored instances.  The main benefit of WMI is it allows you to collect drive space for ALL drives on your monitored instance - otherwise only the drives that contain SQL files are collected.  See the [WMI](/docs/help/wmi) doc for more info.  The [Permissions Helper](/docs/help/permissions-helper) has a button that will grant local admin access.  Or you can use PowerShell.
